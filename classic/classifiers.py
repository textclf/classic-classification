...
class SVMClassifier(TextClassifier):
    ...
    """
    def train(self, kernel='rbf', C=1.0):
        self._train_checks()
        X = self._to_features(self.train_texts)
        self.clf = SVC(kernel=kernel, C=C).fit(X, self.train_labels)
...
class RandomForestTextClassifier(TextClassifier):
    ...
    def grid_search_cv(self, param_grid=None, scoring=None,
        ...
        self._grid_search(RandomForestClassifier(), param_grid, scoring, n_jobs, cv, verbose)
